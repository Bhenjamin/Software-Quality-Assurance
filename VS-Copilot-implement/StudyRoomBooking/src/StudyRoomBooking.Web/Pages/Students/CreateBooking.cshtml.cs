...
public class CreateBookingModel : PageModel
{
    ...
    public CreateBookingModel(IRoomService roomService, IBookingService bookingService, IUserService userService)
    {
    ...
    public async Task<IActionResult> OnPostAsync(int roomId, string bookingDate, string startTime, string endTime, string? notes)
    {
        try
        {
            ...
            // Debug: Log the date being booked
            var today = DateTime.Today;
            var selectedDate = date.Date;
            var daysInAdvance = (selectedDate - today).Days;

            System.Diagnostics.Debug.WriteLine($"DEBUG - Today: {today:yyyy-MM-dd}, Selected: {selectedDate:yyyy-MM-dd}, Days in Advance: {daysInAdvance}");
